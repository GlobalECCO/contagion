...
module.exports = {
  ...
  buildPlayerUpdates: buildPlayerUpdates,
...
function buildPlayerUpdates(update_list, game) {

  // Test if we are still choosing ideologies.
  var choosingIdeology = false;
  for (var player = 0; player < game.players.length; ++player) {
    if (game.players[player].ideologyType === -1) {
      choosingIdeology = true;
    }
  }

  // For each player, set up their payload data.
  for (var player = 0; player < game.players.length; ++player) {
    ...
    } else if ((game.players[player].hasTakenTurn && game.players[player].ideologyType !== -1) ||
    ...
    if (game.phase === GamePhase.ENDGAME) {
      if (player in game.winningPlayers) {
        outcome = "Win";
      } else {
        outcome = "Lose";
      }
    ...
    update_list.push({
    // body.params.payload.update.push({
      "gameInstanceId": game._id,
      "gameTitle": GAME_TITLE,
      "gameVersion": GAME_VERSION,
      "gamingId": game.players[player].name,
      "state": state,
      "outcome": outcome
    });
  }
...
function sendEGSUpdate(rpcIndex, game) {
  ...
  buildPlayerUpdates(body.params.payload.update, game);
  ...
  http_request(opts, function(error, response, body) {
    ...
    //log.debug("Response from EGS: " + body);
